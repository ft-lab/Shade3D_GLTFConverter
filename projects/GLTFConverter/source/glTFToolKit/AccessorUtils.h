...
namespace glTFToolKit
{
    ...
    public:
        ...
        template <typename T>
        static std::pair<std::vector<float>, std::vector<float>> CalculateMinMax(const Microsoft::glTF::Accessor& accessor, const std::vector<T>& accessorContents)
        {
            auto typeCount = Microsoft::glTF::Accessor::GetTypeCount(accessor.type);
